...
def make_all_encoders(name_prefix = u'', make_encoder = make_launch_control_encoder):
    return ([ make_encoder(9 + index, name_prefix + u'_Bottom_Encoder_' + str(index)) for index in xrange(8) ], [ make_encoder(1 + index, name_prefix + u'_Top_Encoder_' + str(index)) for index in xrange(8) ])
...
class TouchOSC(ControlSurface):

    def __init__(self, c_instance):
        super(TouchOSC, self).__init__(c_instance)
    ...
    def disconnect(self):
        super(TouchOSC, self).disconnect()
    ...
    def _do_refresh_state(self):
        ...
        self.schedule_message(3, super(TouchOSC, self).refresh_state)
...
##This is the device mode that you'll want to make the changes to.
    def _init_device(self):
        ...
        make_encoder = partial(make_launch_control_encoder, channel=15)
...
## Change to this next line for rows=
    ...
    def handle_sysex(self, midi_bytes):
        super(TouchOSC, self).handle_sysex(midi_bytes)
